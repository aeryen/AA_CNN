...
class Evaluator:
    ...
    def load(self, dater):
        self.dater = dater
        print("Loading data...")
        self.train_data, self.vocab, self.vocab_inv, _ = self.dater.get_train_data()
        self.test_data, _, _ = self.dater.get_test_data()

    def write_file(self, experiment_dir, checkpoint_step, doc_acc=True, do_is_training=True):
        ...
        with graph.as_default():
            ...
            with sess.as_default():
                ...
                # TRAIN ===========================================================================
                x_batches = DataHelper.batch_iter(self.train_data.value, 64, 1, shuffle=False)
                y_batches = DataHelper.batch_iter(self.train_data.label_instance, 64, 1, shuffle=False)
                all_score = None
                pred_sigmoid = None
                ...
                self.write_dist_file(doc_size_list=self.train_data.doc_size, all_sigmoids=pred_sigmoid,
                                     label=self.train_data.label_doc, experiment_dir=experiment_dir, file_name="train")
            ...
            self.write_dist_file(doc_size_list=self.test_data.doc_size,
                                 all_sigmoids=pred_sigmoid, label=self.test_data.label_doc,
                                 experiment_dir=experiment_dir, file_name="test")

    def write_dist_file(self, doc_size_list, all_sigmoids, label, experiment_dir, file_name):
        # dist_file = open(os.path.join(experiment_dir, file_name + "_dist.txt"), mode="w")
        dist_array = []

        sum_to = 0
        for i in range(len(doc_size_list)):
            f_size = doc_size_list[i]
        ...
        np.savetxt(file_name + "_dist.txt", dist_array)
        np.savetxt(file_name + "_label.txt", label)
...
if __name__ == "__main__":
    ...
    path = sys.argv[1]
    if len(sys.argv) == 2:
        e.write_file(path, step, doc_acc=True, do_is_training=False)
    elif len(sys.argv) > 2:
